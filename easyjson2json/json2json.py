...
class Json2Json(object):
    ...
    def read_list(self, template_field_value, source_field_value):        
        return [self.analyse(template_field_value, source_from_array) for source_from_array in source_field_value]

    def template_is_dict(self, field, template, source):
        source_field_name = self.get_field_name(field, template)                
        ...
        if is_type(source[source_field_name], list):
            return self.read_list(template[field], source[source_field_name])                                                                
        return self.analyse(template[field], source[source_field_name])
    ...
    def template_is_str(self, field, template, source):                
        return get_value(self.get_field_name(field, template), source)
